...
async function getMenu(){
  const res = await fetch(`${process.env.REACT_APP_API_URL}/api/rolls`);
  if(!res.ok) {
    throw new Response('', {status: res.status})
  }
  return res.json();
}
const rollsMenuLoader = async() => {
    return defer({
      menu: getMenu(),
    })
  }
...
const RollsMenu = ()=>{
  ...
  return (
    <>
   <h2 className={style.title}>Меню роллов</h2>
